...
// Function to find an argument on the command line
int findArg(int argc, char *argv[], const std::string& pattern);
/*
 *
...
void addRecord(vector<Record> &db, const Record &newRecord)
{
...
bool updateRecord(vector<Record> &db, const Record &updatedRecord)
{
    for (Record &r : db) {
        if (r.SID == updatedRecord.SID) {
            // Update the record
            r = updatedRecord;
            return true;
        }
...
// Return value should be EXIT_FAILURE if the application exited with any form of error, or EXIT_SUCCESS otherwise
    ...
    if (p) {
        ...
        int sid;
        try {
            // Extract student ID as an integer - can throw an exception if not an integer
            sid = stoi(strID);
    ...
    //**************************************************************
        ...
        if (findArg(argc, argv, "-add")) {
            ...
            if (argc < 10) {
                cerr << "Insufficient parameters to add a record. Usage: -add -sid <SID> -name <NAME> -enrollments <ENROLLMENTS> -grades <GRADES> -phone <PHONE>" << endl;
                return EXIT_FAILURE;
            }

            try {
                // Parse command-line arguments to create a new record
                Record newRecord;
                newRecord.SID = stoi(argv[findArg(argc, argv, "-sid") + 1]);
                newRecord.name = argv[findArg(argc, argv, "-name") + 1];

                stringstream enrollmentsStream(argv[findArg(argc, argv, "-enrollments") + 1]);
                while (!enrollmentsStream.eof()) {
                    string moduleCode;
                    enrollmentsStream >> moduleCode;
                    if (enrollmentsStream.fail()) {
                        break;
                    }
                    newRecord.enrollments.push_back(moduleCode);
                }

                stringstream gradesStream(argv[findArg(argc, argv, "-grades") + 1]);
                while (!gradesStream.eof()) {
                    float grade;
                    gradesStream >> grade;
                    if (gradesStream.fail()) {
                        break;
                    }
                    newRecord.grades.push_back(grade);
                }

                newRecord.phone = argv[findArg(argc, argv, "-phone") + 1];
                ...
                cerr << "Error adding record: " << e.what() << endl;
                return EXIT_FAILURE;
            }
        }
            ...
            // Option to update a record
            if (findArg(argc, argv, "-update")) {
                // Ensure there are enough parameters
                if (argc < 10) {
                    cerr << "Insufficient parameters to update a record. Usage: -update -sid <SID> -name <NAME> -enrollments <ENROLLMENTS> -grades <GRADES> -phone <PHONE>" << endl;
                    return EXIT_FAILURE;
                }

                try {
                    // Parse command-line arguments to create an updated record
                    Record updatedRecord;
                    updatedRecord.SID = stoi(argv[findArg(argc, argv, "-sid") + 1]);
                    updatedRecord.name = argv[findArg(argc, argv, "-name") + 1];
                    ...
                    while (!enrollmentsStream.eof()) {
                        ...
                        updatedRecord.enrollments.push_back(moduleCode);
                    }
                    ...
                    while (!gradesStream.eof()) {
                        ...
                        updatedRecord.grades.push_back(grade);
                    }

                    updatedRecord.phone = argv[findArg(argc, argv, "-phone") + 1];

                    // Update the record in the database
                    if (updateRecord(db, updatedRecord)) {
                        cout << "Record updated successfully." << endl;
                    } else {
                        cout << "Record with SID=" << updatedRecord.SID << " not found. No update performed." << endl;
                    }
